...
function subtitleHtml(plan) {
  return plan.subtitles.map((subtitle, index) => {
    ...
    return `<p id="${attr(subtitle.id)}" class="clip subtitle" data-start="${subtitle.start}" data-duration="${seconds}" data-track-index="${track}" style="--clip-duration:${seconds}s;--clip-delay:${subtitle.start}s;font-size:${subtitle.fontSize}px">${text(subtitle.text)}</p>`;
  }).join('\n      ');
}
...
function calloutHtml(plan) {
  return plan.callouts.map((callout, index) => {
    ...
    return `<h2 id="${attr(callout.id)}" class="clip callout callout-${(index % 2) + 1}" data-start="${callout.start}" data-duration="${seconds}" data-track-index="${6 + (index % 3)}" style="--clip-duration:${seconds}s;--clip-delay:${callout.start}s">${text(callout.text)}${subtext}</h2>`;
  }).join('\n      ');
}

function overlaysHtml(plan) {
  return plan.overlays.map((overlay, index) => {
    const seconds = duration(overlay.start, overlay.end);
    const style = `left:${overlay.position.x}px;top:${overlay.position.y}px;width:${overlay.position.width}px;height:${overlay.position.height}px;--clip-duration:${seconds}s;--clip-delay:${overlay.start}s`;
...
function buildHtml({ config, plan, basePrefix }) {
  const compositionId = 'zz-auto-edit-version-001';
  ...
  const title = `${config.videoTitle} - version-001`;
  const videoSource = assetPath(basePrefix, config.rawVideoPath);
  const realVideo = config.mode === 'real';
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    #stage { position: relative; width: ${config.width}px; height: ${config.height}px; background: radial-gradient(circle at 26% 18%, rgba(66,245,182,.25), transparent 26%), linear-gradient(135deg, #111936 0%, #080b16 60%, #15192b 100%); color: #fff; isolation: isolate; }
    ...
    .avatar::before { content: ""; position: absolute; left: 96px; top: -82px; width: 160px; height: 160px; border-radius: 50%; background: linear-gradient(160deg, #f4d2b8, #b98568); box-shadow: inset -18px -12px 0 rgba(0,0,0,.08); }
    .placeholder-label { position: absolute; left: 758px; top: 752px; width: 404px; text-align: center; font-size: 28px; letter-spacing: .06em; color: rgba(255,255,255,.72); }
    .accent-line { position: absolute; border-radius: 99px; background: var(--accent); box-shadow: 0 0 32px rgba(66,245,182,.65); z-index: 2; }
    .brand { position: absolute; right: 128px; top: 92px; font-size: 26px; color: rgba(255,255,255,.68); letter-spacing: .14em; }
    .callout { position: absolute; left: 154px; top: 118px; max-width: 760px; padding: 22px 30px; border-radius: 28px; font-weight: 900; font-size: 72px; line-height: 1.08; letter-spacing: -.04em; color: #08111f; background: linear-gradient(135deg, #ffffff, #b9ffe7); box-shadow: 0 24px 60px rgba(0,0,0,.28); opacity: 0; transform: translateY(36px) scale(.96); z-index: 5; animation: calloutIn var(--clip-duration) var(--clip-delay) cubic-bezier(.2,.9,.2,1) both; }
    ...
    .subtitle { position: absolute; left: 230px; bottom: 92px; width: 1460px; min-height: 112px; display: grid; place-items: center; padding: 22px 46px; border-radius: 30px; background: rgba(0,0,0,.58); border: 1px solid rgba(255,255,255,.16); box-shadow: 0 18px 54px rgba(0,0,0,.34); font-weight: 800; line-height: 1.28; text-align: center; text-wrap: balance; opacity: 0; transform: translateY(18px); z-index: 6; animation: subtitleIn var(--clip-duration) var(--clip-delay) both; }
    ...
    @keyframes camera { 0%{transform:scale(1)} 25%{transform:scale(min(var(--max-scale),1.05))} 50%{transform:scale(1.015)} 75%{transform:scale(min(var(--max-scale),1.04))} 100%{transform:scale(1.02)} }
    @keyframes calloutIn { 0%{opacity:0;transform:translateY(36px) scale(.96)} 16%,82%{opacity:1;transform:translateY(0) scale(1)} 100%{opacity:0;transform:translateY(-18px) scale(.99)} }
    @keyframes subtitleIn { 0%,100%{opacity:0;transform:translateY(18px)} 10%,88%{opacity:1;transform:translateY(0)} }
    @keyframes fadeWindow { 0%,100%{opacity:0;transform:translateY(14px)} 12%,84%{opacity:1;transform:translateY(0)} }
...
async function main() {
  try {
    ...
    for (const target of files) {
      ...
      await writeFile(target.file, buildHtml({ config, plan, basePrefix: target.basePrefix, gsapPrefix: target.gsapPrefix }), 'utf8');
